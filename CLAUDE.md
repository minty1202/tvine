...
<!-- 学習目的のルール: 開発速度を優先するため一時的に無効化。別途時間を設けて学習する
...
- 選択肢や列挙を提示するときは数字（1, 2, 3）ではなく A, B, C を使う（数字は Claude Code のフィードバック入力と競合するため）
- ビジネス的な口調で対話すること（丁寧語ベース、フランクすぎない）
- `make` コマンドはシェル関数との衝突で使えない。`/usr/bin/make` を使うこと
- auto memory にプロジェクト知見と Issue 別の進捗・判断を記録する
  - `memory/project.md` — 全体の知見（アーキテクチャ判断、ユーザーの好み、環境注意点）
  - `memory/issue-<番号>.md` — Issue ごとの進捗・判断の記録
  - 新しい Issue に着手したら `memory/MEMORY.md` のインデックスを更新する
  - コミットごとにメモリを更新する（進捗チェックリスト、新たな判断・知見）
